...
SELECT purchase_amount, order_no FROM orders ORDER BY purchase_amount DESC;
...
--Activity 8
--Write an SQL statement to find the highest purchase amount ordered by the each customer. Print their ID and highest purchase amount.
...
SELECT customer_id, order_date, MAX(purchase_amount) FROM orders GROUP BY customer_id, order_date HAVING MAX(purchase_amount) IN (2030, 3450, 5760, 6000) ORDER BY customer_id;